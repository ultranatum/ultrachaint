...
#if defined(NDEBUG)
#error "Ultrachaint cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uultrex) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zULTREX is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zULTREX spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zULTREX without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zULTREX spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zULTREX spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zULTREX spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zultrex");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zULTREX mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
        ...
        nSubsidy = 5 * COIN;
	} else if (nHeight < 1000 && nHeight >= 800) {
        nSubsidy = 10 * COIN;
	} else if (nHeight < 2000 && nHeight >= 1000) {
        nSubsidy = 11 * COIN;
	} else if (nHeight < 3000 && nHeight >= 2000) {
        nSubsidy = 12 * COIN;
        ...
        nSubsidy = 30 * COIN;
	} else if (nHeight < 105000 && nHeight >= 100000) {
        nSubsidy = 28 * COIN;
	} else if (nHeight < 110000 && nHeight >= 105000) {
        nSubsidy = 26 * COIN;
	} else if (nHeight < 115000 && nHeight >= 110000) {
        nSubsidy = 24 * COIN;
	} else if (nHeight < 120000 && nHeight >= 115000) {
        nSubsidy = 22 * COIN;
	} else if (nHeight < 125000 && nHeight >= 120000) {
        nSubsidy = 20 * COIN;
	} else if (nHeight < 130000 && nHeight >= 125000) {
        nSubsidy = 18 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZULTREXStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Ultrachaint
...
{
    RenameThread("ultrachaint-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZULTREXMinted()
{
...
void RecalculateZULTREXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zULTREX supply
...
bool RecalculateULTREXSupply(int nHeightStart)
{
...
bool UpdateZULTREXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateULTREXSupply(Params().Zerocoin_StartHeight());
    }

    //Track zULTREX money supply in the block index
    if (!UpdateZULTREXSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zULTREX supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zUltrexSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ULTREX to zULTREX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zULTREX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zULTREX serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zULTREX stake block is less than allowed start height", __func__);

    if (CZUltrexStake* zULTREX = dynamic_cast<CZUltrexStake*>(stake)) {
        CBlockIndex* pindexFrom = zULTREX->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zULTREX stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zULTREX stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zULTREX->GetValue()));
        if (nChecksum200 != zULTREX->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zULTREX->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZULTREX() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zULTREX fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zULTREX mints and %d zULTREX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Ultrachaint: We use certain sporks during IBD, so check to see if they are
