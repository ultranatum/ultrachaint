...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total ultrachaint balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in ultrachaint/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in ultrachaint/kb\n"
    ...
    /* ultrex UniValue zultrexObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zultrexObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zultrexObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zULTREXsupply", zultrexObj)); */
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"ultrachaintaddress\"\n"
            "\nReturn information about the given ultrachaint address.\n"

            "\nArguments:\n"
            "1. \"ultrachaintaddress\"     (string, required) The ultrachaint address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"ultrachaintaddress\", (string) The ultrachaint address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Ultrachaint address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are ultrachaint addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) ultrachaint address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"ultrachaintaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"ultrachaintaddress\"  (string, required) The ultrachaint address to use for the signature.\n"
