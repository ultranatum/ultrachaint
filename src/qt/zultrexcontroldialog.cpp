...
#include "ui_zultrexcontroldialog.h"
...
std::set<std::string> ZUltrexControlDialog::setSelectedMints;
std::set<CMintMeta> ZUltrexControlDialog::setMints;

ZUltrexControlDialog::ZUltrexControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZUltrexControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZUltrexControlDialog::~ZUltrexControlDialog()
{
    delete ui;
}

void ZUltrexControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZUltrexControlDialog::updateList()
{
...
void ZUltrexControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZUltrexControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZUltrex_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZUltrexControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZUltrexControlDialog::GetSelectedMints()
{
...
void ZUltrexControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
