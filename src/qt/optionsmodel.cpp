...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ULTREX);
    ...
    if (!settings.contains("nAnonymizeUltrachaintAmount"))
        settings.setValue("nAnonymizeUltrachaintAmount", 1000);

    nAnonymizeUltrachaintAmount = settings.value("nAnonymizeUltrachaintAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeUltrachaintAmount"))
        SoftSetArg("-anonymizeultrachaintamount", settings.value("nAnonymizeUltrachaintAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ultrachaint.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeUltrachaintAmount:
            return QVariant(nAnonymizeUltrachaintAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeUltrachaintAmount:
            nAnonymizeUltrachaintAmount = value.toInt();
            settings.setValue("nAnonymizeUltrachaintAmount", nAnonymizeUltrachaintAmount);
            emit anonymizeUltrachaintAmountChanged(nAnonymizeUltrachaintAmount);
            break;
        case CoinControlFeatures:
