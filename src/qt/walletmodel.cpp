...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered ultrachaint address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ULTREX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal ultrachaint:URI (ultrachaint:XyZ...?message=example)
            {
...
static void NotifyzULTREXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzULTREXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzULTREXReset.connect(boost::bind(NotifyzULTREXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzULTREXReset.disconnect(boost::bind(NotifyzULTREXReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zultrexTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
