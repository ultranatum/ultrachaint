...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ULTREX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zULTREX send
enum ZerocoinSpendStatus {
    ZULTREX_SPEND_OKAY = 0,                            // No error
    ZULTREX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZULTREX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZULTREX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZULTREX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZULTREX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZULTREX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZULTREX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZULTREX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZULTREX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZULTREX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZULTREX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZULTREX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZULTREX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZULTREX_SPENT_USED_ZULTREX = 14,                      // Coin has already been spend
    ZULTREX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZULTREX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZUltrexBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZULTREXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzultrexAuto) const;
    ...
    */
    ...
    CzULTREXWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzULTREXTracker> zultrexTracker;
    ...
    void setZWallet(CzULTREXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zultrexTracker = std::unique_ptr<CzULTREXTracker>(new CzULTREXTracker(strWalletFile));
    }

    CzULTREXWallet* getZWallet() { return zwalletMain; }
    ...
    void setZUltrexAutoBackups(bool fEnabled)
    {
    ...
    /** zULTREX reset */
    boost::signals2::signal<void()> NotifyzULTREXReset;
