...
CzULTREXWallet::CzULTREXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zultrex seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZULTREXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZULTREXSeed_deprecated()) {
                    LogPrintf("%s: Updated zULTREX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zultrex seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zultrex wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzULTREXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZULTREXCount(nCountLastUsed);
    else if (!walletdb.ReadZULTREXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzULTREXWallet::Lock()
{
    seedMaster = 0;
}

void CzULTREXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzULTREXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZULTREX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzULTREXWallet::LoadMintPoolFromDB()
{
...
void CzULTREXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzULTREXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzULTREXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zultrexTracker->HasPubcoinHash(pMint.first)) {
...
bool CzULTREXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZULTREX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zultrexTracker which also adds to database
    pwalletMain->zultrexTracker->Add(dMint, true);
...
void CzULTREXWallet::SeedToZULTREX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzULTREXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzULTREXWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZULTREXCount(nCountLastUsed);
...
void CzULTREXWallet::GenerateDeterministicZULTREX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzULTREXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzULTREXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
